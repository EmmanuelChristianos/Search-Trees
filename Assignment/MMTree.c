...
static void doCompareTree(Link root, Link new);
...
static void doCompareTree(Link root, Link new) {
    ...
    if(strcmp(new->msgid,root->msgid) > 0){
        
        if(root->right != NULL){
            
        doCompareTree(root->right,new);
    ...
    } else if (strcmp(new->msgid,root->msgid) < 0){
        
        if(root->left != NULL) {
        
            doCompareTree(root->left,new);
